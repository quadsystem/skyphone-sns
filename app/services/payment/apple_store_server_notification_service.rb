...
class Payment::AppleStoreServerNotificationService < BaseService
  include Payment::Helper

  # @param payload [Payment::AppleNotificationInfo]
  def call(payload)
    @payload = payload
    ...
    return if Payment::ApplePurchaseTransaction.where(notification_uuid: @payload.notification_uuid).exists?
  ...
  def create_transaction
    ...
    Payment::ApplePurchaseTransaction.create(transaction)
  end

  def update_account_transaction()
    transaction_id = @payload.transaction_info['originalTransactionId']
    account_transaction = Payment::PurchaseTransaction.where(transaction_id: transaction_id).first
