class Payment::GooglePurchaseService < BaseService
  def call(account, params)
    ...
    @package_name = Rails.application.config_for(:payment)['android_package_name']
    @service = 'google'
    ...
    raise Payment::PurchaseError, "invalid orderId" if @subscription.order_id.nil? || @subscription.order_id.empty?
  ...
  def validate_params(params)
    raise Payment::PurchaseError, 'subscription_id is empty' if params['subscription_id'].nil?
    raise Payment::PurchaseError, 'purchase_token is empty' if params['purchase_token'].nil?
  end

  # @param account_transaction [Payment::PurchaseTransaction]
  def check_transaction(account_transaction)
    if account_transaction.present? \
      && account_transaction.account_id != @account.id
      raise Payment::PurchaseError, 'already used transaction'
    end
  end
