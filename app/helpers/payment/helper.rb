module Payment::Helper
  ...
  def check_ios_bundle_id?(bundle_id)
    puts "config.bundle.id: #{Rails.application.config_for(:payment)['ios_bundle_id']}"
    bundle_id == Rails.application.config_for(:payment)['ios_bundle_id']
  end
